  ...
  return (
    <AnimatePresence mode="wait">
      <motion.section
        ...
        <div className="w-full flex flex-row items-center justify-center gap-10">
          {dataLayout.map((item) => (
            <div
              key={item.id}
              className={`grid grid-rows-1 lg:grid-cols-1 gap-10 items-center justify-center border border-slate-100 p-4 shadow-md cursor-pointer rounded-md ${
                item.id === show.id
      ...
      <section className="w-full flex flex-col">
        <div className="bg-sky-800 p-20">Testing</div>
        <div>Testing</div>
      </section>
    </AnimatePresence>
  );
